...
class DocenteController {
    static async obtenerCargaAcademica(req, res) {
        try {
            const docente = await prisma_1.prisma.perfilDocente.findUnique({
                where: { usuarioId: req.usuario.sub },
            });
            if (!docente) {
                return res.status(404).json({ mensaje: 'Perfil de docente no encontrado' });
            }
            const asignaciones = await prisma_1.prisma.cursoMateria.findMany({
    ...
    static async obtenerEstudiantesDelCurso(req, res) {
        try {
            const docente = await prisma_1.prisma.perfilDocente.findUnique({
                where: { usuarioId: req.usuario.sub },
            });
            ...
            const cursoMateria = await prisma_1.prisma.cursoMateria.findFirst({
            ...
            const estudiantes = await prisma_1.prisma.perfilEstudiante.findMany({
    ...
    static async crearPlanEvaluacion(req, res) {
        try {
            const docente = await prisma_1.prisma.perfilDocente.findUnique({
                where: { usuarioId: req.usuario.sub },
            });
            ...
            const plan = await prisma_1.prisma.planEvaluacion.create({
                data: {
                    titulo: data.titulo,
    ...
    static async obtenerPlanes(req, res) {
        try {
            ...
            const planes = await prisma_1.prisma.planEvaluacion.findMany({
                where: {
                    ...(cursoMateriaId ? { cursoMateriaId: cursoMateriaId } : {}),
    ...
    static async registrarCalificacion(req, res) {
        try {
            const docente = await prisma_1.prisma.perfilDocente.findUnique({
                where: { usuarioId: req.usuario.sub },
            });
            ...
            const data = req.body;
            const plan = await prisma_1.prisma.planEvaluacion.findFirst({
                where: {
            ...
            const estudiante = await prisma_1.prisma.perfilEstudiante.findUnique({
            ...
            const calificacionExistente = await prisma_1.prisma.calificacion.findFirst({
                where: {
            ...
            if (calificacionExistente) {
                calificacion = await prisma_1.prisma.calificacion.update({
                ...
                calificacion = await prisma_1.prisma.calificacion.create({
                    data: {
                        puntaje: data.puntaje,
                        retroalimentacion: data.retroalimentacion,
                        estudianteId: data.estudianteId,
    ...
    static async registrarAsistencias(req, res) {
        try {
            const docente = await prisma_1.prisma.perfilDocente.findUnique({
                where: { usuarioId: req.usuario.sub },
            });
            ...
            await prisma_1.prisma.asistencia.createMany({
